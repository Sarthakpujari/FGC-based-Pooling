...
def train(model, loader, optimizer):
    ...
    cnt=1
    for data in loader:
        print(cnt," out of 2400 i think")
        cnt+=1
        data = data.to(device)
        optimizer.zero_grad()
        out, aux_loss = model(data)
        if(args.pooling=='fgc'):
            loss = F.nll_loss(out, data.y.view(-1),reduction='mean') + aux_loss
            print("loss is ",loss)
            loss.backward()
            optimizer.step()
...
### Dataset
# path = "data/EXPWL1/"
# path = "D:/graph_pooling/fgcexpressivepower/The-expressive-power-of-pooling-in-GNNs-main/data/EXPWL1"
path = "C:/Users/HP/Downloads/The-expressive-power-of-pooling-in-GNNs-main-rahul/The-expressive-power-of-pooling-in-GNNs-main/data/EXPWL1"
...
for r in range(args.runs):  
    ...
    #uncomment following for original
    
    # train_dataset = dataset[len(dataset) // 5:]
    # val_dataset = dataset[:len(dataset) // 10]
    # test_dataset = dataset[len(dataset) // 10:len(dataset) // 5]
    
    #taking smaller subset
    
    train_dataset = dataset[len(dataset) // 50:]
    val_dataset = dataset[:len(dataset) // 100]
    test_dataset = dataset[len(dataset) // 10:len(dataset) // 5]
